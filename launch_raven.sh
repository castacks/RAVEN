...
tmux send-keys -t "$SESSION:rayfronts" "docker run -it --rm --name rayfronts_container --gpus all --network host --ipc host --privileged --runtime=nvidia -e NVIDIA_DRIVER_CAPABILITIES=all -e ROS_DOMAIN_ID=1 -e ISAAC_SIM_SCRIPT_NAME -v $RAVEN/RayFronts:/workspace/RayFronts -v $RAVEN/AirStack/simulation/isaac-sim/launch_scripts:/workspace/launch_scripts -w /workspace/RayFronts rayfronts:desktop bash -i /workspace/RayFronts/run_mapping_server_rosnode.sh" Enter
